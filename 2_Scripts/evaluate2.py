...
# Now you can import your custom classes and functions from Model 2
from Model_2_final import create_resnet18_model, GalaxyDataset, val_transform
...
# Define target columns from the dataframe that was just loaded
target_cols = [col for col in test_df.columns if col.endswith("_debiased")]

test_image_paths = test_df['file_path'].tolist()
test_labels = test_df[target_cols].to_numpy(dtype='float32')
...
model = create_resnet18_model(pretrained=False) # Use pretrained=False
